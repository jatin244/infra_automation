...
output "redis_endpoint" {
  value       = var.cluster_enabled ? join("", aws_elasticache_cluster.default.*.cache_nodes.0.address) : (var.cluster_replication_enabled ? join("", aws_elasticache_replication_group.cluster.*.configuration_endpoint_address) : join("", aws_elasticache_replication_group.default.*.primary_endpoint_address))
...
output "memcached_endpoint" {
  value       = var.cluster_enabled ? join("", aws_elasticache_cluster.default.*.cache_nodes.0.address) : ""
