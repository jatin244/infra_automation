...
resource "aws_cloudwatch_log_group" "default" {
  provider          = aws.stack
  count             = var.enable_elasticache_cluster && length(var.log_delivery_configuration) > 0 ? 1 : 0
...
resource "aws_elasticache_replication_group" "default" {
  provider                   = aws.stack
  count                      = var.enable_elasticache_cluster && var.replication_enabled ? 1 : 0
  engine                     = var.engine
...
resource "aws_elasticache_replication_group" "cluster" {
  provider                   = aws.stack
  count                      = var.enable_elasticache_cluster && var.cluster_replication_enabled ? 1 : 0
  engine                     = var.engine
  replication_group_id       = module.labels.id
  description                = module.labels.id
  engine_version             = var.engine_version
...
resource "aws_elasticache_cluster" "default" {
  provider             = aws.stack
  count                = var.enable_elasticache_cluster && var.cluster_enabled ? 1 : 0
  engine               = var.engine
