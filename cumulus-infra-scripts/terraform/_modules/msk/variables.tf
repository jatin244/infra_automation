...
variable "aws_msk_serverless_cluster_enabled" {
  type        = bool
  default     = false
  description = "Enables SCRAM client authentication via AWS Secrets Manager"
}
variable "serverless_cluster_iam_authentication_enabled" {
  type    = bool
  default = false
}
...
variable "msk_cluster_cloudwatch_log_group_tags" {
  type    = map(any)
  default = {}
}
...
variable "auto_scaling_storage_enabled" {
  type        = bool
  default     = true
  description = "Enable or Disable Auto scaling for storage"
}
